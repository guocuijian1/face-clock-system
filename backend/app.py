...
@app.route('/getFirstImage', methods=['GET'])
def get_first_image():
    try:
        image_service.load_images()
        image_meta = image_service.get_first_image()
        if not image_meta:
            return jsonify({'error': 'No images available'}), 404

        return jsonify({
            'name': image_meta['name'],
            'job_id': image_meta['job_id'],
            'image_binary': image_meta['image_binary']  # Convert numpy array to bytes
        }), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/getNextImage', methods=['GET'])
def get_next_image():
    try:
        image_meta = image_service.get_next_image()
        if not image_meta:
            return jsonify({'error': 'No images available'}), 404
        ...
        return jsonify({
            'name': image_meta['name'],
            'job_id': image_meta['job_id'],
            'image_binary': bytes(image_meta['image_binary']).hex()  # Convert numpy array to bytes
        }), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/getPreviousImage', methods=['GET'])
def get_previous_image():
    try:
        image_meta = image_service.get_previous_image()
        if not image_meta:
            return jsonify({'error': 'No images available'}), 404
        ...
        return jsonify({
            'name': image_meta['name'],
            'job_id': image_meta['job_id'],
            'image_binary': image_meta['image_binary'].hex()
        }), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def face_locations():
    try:
        ...
        # 检测人脸位置
        face_locations_result = image_service.get_face_locations(base64_string)
        ...
        # 裁剪检测到的人脸
        face_image_base64 = image_service.crop_and_save_faces(
            base64_string,
            face_locations_result
        )
        ...
        return jsonify({
            ...
            'message': 'Successfully detected and processed face'
        }), 200
...
def get_cropped_faces():
    try:
        ...
        # 使用ImageService裁剪人脸
        face_image_base64 = image_service.crop_and_save_face(base64_string, face_locations_result[0])
        if not face_image_base64:
            return jsonify({
                'status': 500,
                'message': 'Failed to process face image',
